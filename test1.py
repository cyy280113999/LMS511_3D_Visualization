...
CommentList = ['Command Type','Command','Version Number','Device Number','Serial Number',
...
#        add Data
CommentList=CommentList+['Data']*(int(datas[CommentList.index('Amount of Data')],16))
#        add Space until the end
CommentList=CommentList+['???']*(len(datas)-len(CommentList))
bindDatas = [[index,comment,data]for index,comment,data in zip(range(len(datas)),CommentList,datas)]
...
TelDict = {}
CommentIndex = datas.index('DIST1')
# TelDict['Content']=CommentIndex  # Not necessarily
AfterList = CommentList[CommentIndex:CommentList.index('Amount of Data')+1]
for index,afterComment in zip(range(len(AfterList)),AfterList):
    TelDict[afterComment]=CommentIndex+index
TelDict['Data Start'] = TelDict['Amount of Data']+1
print(TelDict)


factorComment = datas[TelDict['Scale Factor']]
factors=0
if factorComment == '40000000':
    factors = 2
elif factorComment == '3F800000':
    factors = 1
else:
    exec('error')
